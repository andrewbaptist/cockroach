...
func compareEntryLeases(a, b *CacheEntry) int {
	...
	// For expiration based leases, we don't update the sequence when we update
	// the ExpirationTime.
	if a.Lease().Type() == roachpb.LeaseExpiration && b.Lease().Type() == roachpb.LeaseExpiration {
		return a.Lease().GetExpiration().Compare(b.Lease().GetExpiration())
	}
	if a.Lease().Type() != roachpb.LeaseEpoch && b.Lease().Type() == roachpb.LeaseEpoch {
		// NB: Today when we convert to an Expiration lease, we update the sequence.
		// Generally this is a one way conversion so consider an Epoch lease as
		// newer.
		return -1
	}
	if a.Lease().Type() == roachpb.LeaseEpoch && b.Lease().Type() != roachpb.LeaseEpoch {
...
func (e *CacheEntry) maybeUpdate(
	...
	if compareEntryLeases(newEntry, e) > 0 {
		newEntry.lease = *l
		updatedLease = true
	}
